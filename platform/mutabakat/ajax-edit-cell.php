...
try {
    ...
    // Guncelle
    if ($field === 'policy_no') {
        $st = $conn->prepare("UPDATE mutabakat_v2_rows SET policy_no=?, policy_no_norm=?, updated_at=NOW() WHERE id=?");
        $st->bind_param('ssi', $value, $policyNorm, $rowId);
    } else {
        $st = $conn->prepare("UPDATE mutabakat_v2_rows SET $field=?, updated_at=NOW() WHERE id=?");
        $st->bind_param('si', $value, $rowId);
    }
