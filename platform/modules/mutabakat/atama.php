    ...
    if ($action === 'create_assignment' && $isMain) {
        ...
        if ($taliId <= 0) {
            ...
            try {
                ...
                $st = $conn->prepare("SELECT a.id, a.txn_type, a.araci_kom_payi FROM mutabakat_v2_rows a JOIN mutabakat_v2_matches m ON m.ana_row_id=a.id AND m.period_id=a.period_id AND m.status='MATCHED' JOIN mutabakat_v2_rows t ON t.id=m.tali_row_id LEFT JOIN mutabakat_v2_assignment_rows arw ON arw.row_id=a.id WHERE a.id IN ($placeholders) AND a.period_id=? AND a.ana_acente_id=? AND a.source_type='ANA_CSV' AND t.tali_acente_id=? AND arw.row_id IS NULL");
                $params = array_merge($rowIds, [$periodId, $mainAgencyId, $taliId]);
                $st->bind_param($types . 'iii', ...$params);
                $st->execute();
                $res = $st->get_result();
                $validRows = [];
                ...
                if (count($validRows) !== count($rowIds)) {
                    throw new Exception('Secilen satirlardan bazilari bu tali acenteye ait eslesme degil veya daha once atanmis.');
                }
...
<div class="container">
    ...
    <div class="card-header">
      ...
      <form method="get">
        ...
        <?php if ($isMain && !empty($taliAcenteler)): ?>
          <label class="u-text-sm u-text-muted">Tali Acente</label>
      ...
      <table class="excel-table">
        ...
        <tbody>
          <?php foreach ($pendingAssignments as $pa): ?>
            <tr>
              <td>
                <a href="?period_id=<?= $periodId ?>&view=<?= $pa['id'] ?>">#<?= h($pa['id']) ?></a>
              </td>
              <td><?= h($pa['assigned_at']) ?></td>
              <td><?= h($pa['row_count']) ?></td>
              <td><?= _money_span($pa['summary_hakedis']) ?></td>
              <td><?= h($pa['notes'] ?? '-') ?></td>
              <td>
                <div class="action-btn-group">
                  <form method="post" onsubmit="return confirm('Atamayi onaylamak istediginize emin misiniz?');">
                    <input type="hidden" name="action" value="approve_assignment">
                    <input type="hidden" name="assignment_id" value="<?= $pa['id'] ?>">
                    <button type="submit" class="btn btn-success">Onayla</button>
                  </form>
                  <button type="button" class="btn btn-danger" onclick="showRejectModal(<?= $pa['id'] ?>)">Reddet</button>
                  <button type="button" class="btn" onclick="showDisputeModal(<?= $pa['id'] ?>)">Itiraz</button>
                </div>
              </td>
            </tr>
          <?php endforeach; ?>
  ...
  <div class="card">
    <div>
      <div>Atanabilir Satirlar (<?= count($assignableRows) ?>)</div>
      ...
      <form method="post" id="assignForm">
        ...
        <input type="text" name="notes" class="form-control" placeholder="Not (opsiyonel)">
        <button type="submit" class="btn btn-success" id="btnAssign" disabled>Secilenleri Ata</button>
        <span id="selectedCount">0 secili</span>
      </form>
      <?php endif; ?>
    </div>
    ...
    <div class="table-wrapper">
      <table class="excel-table">
        ...
        <tbody>
          <?php if (empty($assignableRows)): ?>
            <tr><td colspan="8">Atanabilir satir yok.</td></tr>
          <?php else: ?>
      ...
      <table class="excel-table">
        ...
        <tbody>
          <?php if (empty($allAssignments)): ?>
            <tr><td colspan="<?= $isMain ? 11 : 10 ?>">Atama yok.</td></tr>
          <?php else: ?>
            ...
            ?>
              <tr>
                ...
                <td><?= h($a['row_count']) ?></td>
                <td><?= _money_span($a['summary_satis']) ?></td>
                <td><?= _money_span($a['summary_iptal']) ?></td>
                <td><?= _money_span($a['summary_zeyil']) ?></td>
                <td><?= _money_span($a['summary_hakedis']) ?></td>
                <td><span class="status-badge <?= $statusClass ?>"><?= h($a['status']) ?></span></td>
                <td>
                  <a href="?period_id=<?= $periodId ?>&view=<?= $a['id'] ?>" class="btn">Goster</a>
                </td>
              </tr>
              <?php if (!empty($a['rejection_reason'])): ?>
              <tr>
                <td colspan="<?= $isMain ? 11 : 10 ?>">
  ...
  <div class="modal-content">
    ...
    <form method="post">
      ...
      <div>
        ...
        <button type="submit" class="btn">Itiraz Et</button>
      </div>
    </form>
  </div>
</div>
  ...
  <div class="modal-content">
    ...
    <div>
      ...
      <div class="detail-row">
        ...
        <div class="detail-value"><?= _money_span($assignmentDetail['summary_hakedis']) ?> TRY</div>
      </div>
      ...
      <div class="detail-row">
        <div class="detail-label">Red Sebebi:</div>
        <div class="detail-value"><?= h($assignmentDetail['rejection_reason']) ?></div>
      </div>
      <?php endif; ?>
      <?php if (!empty($assignmentDetail['tali_notes'])): ?>
      <div class="detail-row">
        <div class="detail-label">Tali Notu:</div>
        <div class="detail-value"><?= h($assignmentDetail['tali_notes']) ?></div>
      </div>
      <?php endif; ?>
    </div>

    <div>Atanan Satirlar (<?= count($assignmentRows) ?>)</div>
    <div class="table-wrapper">
      <table class="excel-table">
        <thead>
          <tr>
        ...
        <tbody>
          <?php foreach ($assignmentRows as $ar): ?>
            <tr>
              ...
              <td><?= _money_span($ar['araci_kom_payi']) ?></td>
            </tr>
          <?php endforeach; ?>
        </tbody>
      </table>
