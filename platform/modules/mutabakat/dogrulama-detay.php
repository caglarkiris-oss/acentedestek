    ...
    <div class="card2">
      <div class="head">
        ...
        <div class="right">
          ...
          <button class="btn btn-blue" type="button" id="btnFinalize" <?= $finalizeDisabled ? 'disabled' : '' ?>>
            Mutabakat Tamamla
          </button>

          <form id="uploadForm" method="post" enctype="multipart/form-data" action="<?= h($selfUrl) ?>">
            <input type="hidden" name="action" value="upload_csv">
            <input type="hidden" name="csrf_token" value="<?= h($csrfToken) ?>">
            <input id="csvFile" type="file" name="csv_file" accept=".csv,text/csv">
      ...
      <?php if ($flashErr): ?>
        <div class="alert bad"><?= h($flashErr) ?></div>
      <?php elseif ($flashOk): ?>
        <div class="alert ok"><?= h($flashOk) ?></div>
      <?php endif; ?>

      <?php if($subMainFinal===1): ?>
        <div class="alert ok">Mutabakat tamamlandı. Ödemeler açıldı.</div>
      <?php endif; ?>
        ...
        <table>
          ...
          <tbody>
            <?php if (empty($cacheRows)): ?>
              <tr><td colspan="<?= count($TEMPLATE_HEADERS)+2 ?>">Henüz veri yok.</td></tr>
            <?php else: ?>
