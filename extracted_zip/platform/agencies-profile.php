...
<main class="page">
  <section class="card">
    ...
    <form method="post" novalidate>
      ...
      <h3 style="margin:0 0 10px;font-size:14px;font-weight:850;">Çalışılan Sigorta Şirketleri</h3>

      <?php if ($hasCompanies): ?>
        <div class="grid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(240px,1fr));gap:10px">
          <?php foreach ($companies as $c): ?>
            <label class="check" style="display:flex;align-items:center;gap:10px;padding:12px;border:1px solid rgba(15,23,42,.10);border-radius:14px;background:#fff;">
              <input
                type="checkbox"
                name="companies[]"
                value="<?= (int)$c['id'] ?>"
              ...
              >
              <span style="font-weight:750;"><?= e($c['name']) ?></span>
            </label>
          <?php endforeach; ?>
        </div>
        <?php if (!$hasPivot): ?>
          <div class="small" style="margin-top:10px;">Not: agency_companies tablosu olmadığı için seçimler kaydedilemez.</div>
        <?php endif; ?>
      <?php else: ?>
